...
class KakaoDto {
    ...
    public static class KakaoTokenResponse {
        ...
        public AuthInfo.TokenInfo toTokenInfo() {
            return new AuthInfo.TokenInfo(
                    access_token,
                    refresh_token,
                    LocalDateTime.now().plusSeconds(expires_in),
                    LocalDateTime.now().plusSeconds(refresh_token_expires_in)
            );
        }
    }
    ...
    public static class KakaoAccountInformation {
        ...
        public AuthInfo.OAuthUserInfo toOAuthUserInfo() {
            if (id == null) {
                throw new OauthProcessingErrorException("카카오에서 회원번호가 오지 않았습니다.");
            }
            return new AuthInfo.OAuthUserInfo(
                    id,
                    kakao_account != null ? kakao_account.getName() : null,
                    Optional.ofNullable(kakao_account)
                            .map(KakaoAccount::getGender)
                            .map(KakaoGender::toProtoGender)
                            .orElse(Gender.GENDER_UNSPECIFIED),
                    kakao_account != null ? kakao_account.getBirthyear() : null,
                    kakao_account != null ? kakao_account.getPhone_number() : null
            );
        }
